...
class MasterDataController extends Controller
{
    ...
    {
        if ($supplier->items()->exists() || GEOrder::where("supplier_id", $supplier->id)->exists() || PurchaseOrder::where("supplier_id", $supplier->id)->exists()) return back()->with("error", "This supplier is referenced by items or purchasing records and cannot be deleted.");
        $supplier->delete();
    ...
    {
        if ($item->branches()->exists() || GEOrderItem::where("item_id", $item->id)->exists() || PurchaseOrderItem::where("item_id", $item->id)->exists()) return back()->with("error", "This item is referenced by inventory or purchase records and cannot be deleted.");
        $item->delete();
