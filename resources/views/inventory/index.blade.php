<x-app-layout title="Inventory">
    ...
    <section class="card animate-fade-in">
        <form method="GET" action="{{ route('inventory.index') }}" class="border-b border-slate-200 p-5">
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
                ...
                <select name="status" class="input">
                    <option value="">All stock statuses</option>@foreach($statusLabels as $value => $label)<option value="{{ $value }}" @selected($selectedStatus===$value)>{{ $label }}</option>@endforeach
                </select>
            </div>
        ...
        <x-table-section title="Inventory records" :count="$inventory->count()" :open="true">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $itemBranch->location ?: '—' }}</td>
                            <td class="text-right font-semibold tabular-nums">{{ $itemBranch->current_stock }}</td>
                            <td class="text-right tabular-nums">K {{ number_format((float) $itemBranch->unit_cost, 2) }}</td>
                            <td class="text-right font-medium tabular-nums">K {{ number_format($itemBranch->inventoryValue(), 2) }}</td>
                            <td><span class="tabular-nums">{{ $itemBranch->reorder_level }}</span>
