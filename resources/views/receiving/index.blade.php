<x-app-layout title="Purchase Receipts">
    <x-page-header title="Purchase Receipts" description="Record supplier deliveries and post stock receipts.">
        <x-slot name="actions">
            <a href="{{ route('receiving.export', 'pdf') }}" class="btn btn-secondary" title="Export PDF"><i data-lucide="file-text" class="h-4 w-4"></i> PDF</a>
            <a href="{{ route('receiving.export', 'xlsx') }}" class="btn btn-secondary" title="Export Excel"><i data-lucide="sheet" class="h-4 w-4"></i> Excel</a>
            @can('create', App\Models\PurchaseReceipt::class)
            <a href="{{ route('receiving.create') }}" class="btn btn-primary"><i data-lucide="package-check" class="h-4 w-4"></i> Receive</a>
            @endcan
        </x-slot>
    </x-page-header>
        ...
        <div class="table-wrap">
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><a href="{{ route('receiving.show', $receipt) }}" class="font-mono text-xs font-semibold text-brand-600">{{ $receipt->receipt_number }}</a></td>
                        ...
                        <td class="text-right"><a href="{{ route('receiving.show', $receipt) }}" class="text-sm font-medium text-brand-600">View</a></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
