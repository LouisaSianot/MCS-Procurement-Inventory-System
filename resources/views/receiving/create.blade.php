<x-app-layout title="Receive Purchase Order">
    ...
    <section class="card animate-fade-in">
        <div class="border-b border-slate-200 p-5">
            <h2 class="text-base font-semibold">Select a purchase order</h2>
            <p class="mt-1 text-sm text-slate-500">Only ordered and partially received purchase orders can be received.</p>
        </div>
        ...
        @else <x-table-section title="Available purchase orders" description="Choose an ordered purchase order to record delivery." :count="$purchaseOrders->count()" :open="true">
            <div class="table-wrap">
                <table class="data-table">
                    ...
                    <tbody>@foreach ($purchaseOrders as $purchaseOrder)<tr>
                            <td class="font-mono text-xs font-semibold">{{ $purchaseOrder->po_number }}</td>
                            <td>{{ $purchaseOrder->supplier->name }}</td>
                            <td>{{ $purchaseOrder->branch->name }}</td>
                            <td><x-status-badge :status="$purchaseOrder->geOrder->inventory_flag" /></td>
                            <td class="text-right"><a href="{{ route('receiving.create', ['purchase_order_id' => $purchaseOrder->id]) }}" class="btn btn-primary py-2">Select</a></td>
                        </tr>@endforeach</tbody>
                </table>
            </div>
    ...
    <form method="POST" action="{{ route('receiving.store') }}">@csrf
        ...
        <section class="card animate-fade-in">
            <div class="border-b border-slate-200 p-5">
                <h2 class="text-base font-semibold">Receipt details</h2>
            </div>
            <div class="grid grid-cols-1 gap-5 p-5 sm:grid-cols-2 lg:grid-cols-3">
                <x-form-field name="receipt_number" label="Receipt Number" :value="old('receipt_number', $receiptNumber)" :errors="$errors" placeholder="e.g. GRN-2026-001" required />
                <x-form-field name="received_at" label="Received Date" type="date" :value="old('received_at', $defaultDate)" :errors="$errors" required />
                <x-form-field name="supplier_delivery_reference" label="Supplier Delivery Reference" :value="old('supplier_delivery_reference')" :errors="$errors" placeholder="Invoice or delivery note number" />
                <div>
                    <p class="text-sm font-medium text-slate-700">Purchase Order</p>
                    <p class="mt-1.5 rounded-lg bg-slate-50 px-3.5 py-2.5 font-mono text-sm">{{ $selectedPurchaseOrder->po_number }}</p>
                </div>
                <div>
        ...
        <section class="card animate-fade-in mt-6">
            <div class="border-b border-slate-200 p-5">
                <h2 class="text-base font-semibold">Delivered line items</h2>
                <p class="mt-1 text-sm text-slate-500">Enter only quantities delivered in this receipt.</p>
            </div>
            <div class="table-wrap">
                <table class="data-table">
                    <thead>
