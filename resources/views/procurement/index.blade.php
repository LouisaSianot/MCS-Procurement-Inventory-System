        ...
        <form method="GET" action="{{ route('procurement.index') }}" class="border-b border-slate-200 p-5">
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <select name="status" class="input">
                    <option value="">All statuses</option>@foreach (['draft' => 'Draft', 'ordered' => 'Ordered', 'backorder' => 'Backorder', 'partially received' => 'Partially Received', 'fully received' => 'Fully Received', 'cancelled' => 'Cancelled'] as $value => $label)<option value="{{ $value }}" @selected($status===$value)>{{ $label }}</option>@endforeach
                </select>
        ...
        <div class="table-wrap">
            <table class="data-table hidden md:table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><a href="{{ route('procurement.show', $purchaseOrder) }}" class="font-mono text-xs font-semibold text-brand-600 hover:text-brand-700">{{ $purchaseOrder->po_number }}</a></td>
                        <td><a href="{{ route('ge-orders.show', $purchaseOrder->geOrder) }}" class="font-mono text-xs text-slate-600 hover:text-brand-600">{{ $purchaseOrder->geOrder->order_number }}</a></td>
                        <td>{{ $purchaseOrder->supplier->name }}</td>
                        <td class="text-slate-500">{{ $purchaseOrder->order_date->format('d M Y') }}</td>
                        <td class="text-right font-medium tabular-nums">K {{ number_format((float) $purchaseOrder->total_amount, 2) }}</td>
                        <td><x-status-badge :status="$purchaseOrder->status" /></td>
                        <td class="text-right"><a href="{{ route('procurement.show', $purchaseOrder) }}" class="inline-flex items-center justify-center rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-brand-600" title="View"><i data-lucide="eye" class="h-4 w-4"></i></a></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            <div class="divide-y divide-slate-100 md:hidden">@foreach ($orders as $purchaseOrder)<div class="p-4">
                    <div class="flex items-start justify-between gap-3">
                        <div><a href="{{ route('procurement.show', $purchaseOrder) }}" class="font-mono text-sm font-semibold text-brand-600">{{ $purchaseOrder->po_number }}</a>
                            <p class="mt-0.5 text-xs text-slate-500">{{ $purchaseOrder->supplier->name }} · {{ $purchaseOrder->geOrder->order_number }}</p>
                        </div><x-status-badge :status="$purchaseOrder->status" />
                    </div>
                    <p class="mt-3 text-sm font-semibold tabular-nums text-slate-900">K {{ number_format((float) $purchaseOrder->total_amount, 2) }}</p>
                </div>@endforeach</div>
        </div>
