<x-app-layout title="Create Purchase Order">
    <x-page-header title="Create Purchase Order" description="Generate a supplier Purchase Order from an approved GE Order." :breadcrumbs="[['label' => 'Purchase Orders', 'url' => route('procurement.index')], ['label' => 'Create']]">
        <x-slot name="actions"><a href="{{ route('procurement.index') }}" class="btn btn-secondary"><i data-lucide="arrow-left" class="h-4 w-4"></i> </a></x-slot>
    </x-page-header>

    @if (! $selectedGEOrder)
    <section class="card animate-fade-in">
        <div class="border-b border-slate-200 p-5">
            <h3 class="text-base font-semibold text-slate-900">Select an approved GE Order</h3>
        ...
        <section class="card animate-fade-in">
            ...
            <div class="grid grid-cols-1 gap-5 p-5 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div>
                    ...
                    <p class="mt-1.5 rounded-lg bg-slate-50 px-3.5 py-2.5 text-sm text-slate-700">{{ $selectedGEOrder->supplier->name }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-slate-700">Branch</p>
                    <p class="mt-1.5 rounded-lg bg-slate-50 px-3.5 py-2.5 text-sm text-slate-700">{{ $selectedGEOrder->branch->name }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-slate-700">GE Order</p>
                    <p class="mt-1.5 font-mono text-sm font-semibold text-brand-600">{{ $selectedGEOrder->order_number }}</p>
                </div>
